...
for route_id_dir in route_id_dir_list[10:15]:
    ...
    _train = overall_df.query("transit_date >= @train_dates[0] and transit_date <= @train_dates[1]")
    _test = overall_df.query("transit_date >= @test_dates[0] and transit_date <= @test_dates[1]")
    _val = overall_df.query("transit_date >= @val_dates[0] and transit_date <= @val_dates[1]")
    if len(_train) == 0 or len(_test) == 0 or len(_val) == 0:
        continue
    print(route_id_dir, len(_train), len(_test), len(_val))
